...
//  - velocita' lineare nel frame WORLD-FLU fissato alla direzione di marcia iniziale
//  - velocita' angolare nel frame body FLU
// Usa:
//  - /fmu/out/vehicle_odometry per ricavare yaw iniziale (dal quaternione),
//    velocita' lineare (NED) e angolare (body FRD)
...
private:
  void vehicle_odom_cb(const px4_msgs::msg::VehicleOdometry::SharedPtr msg) {
    ...
    if (!yaw_offset_initialized_) {
      // PX4 VehicleOdometry.q e' NED->FRD (w,x,y,z)
  ...
  void try_publish() {
    ...
    // Velocita' lineare: campo velocity in NED [N, E, D]
    ...
    // Velocita' angolare: campo angular_velocity in frame body FRD
